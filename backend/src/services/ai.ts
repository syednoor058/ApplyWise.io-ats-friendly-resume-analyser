  ...
  analysisResult: z.object({
    overall: z.object({
      ...
      features: z.array(z.object({
        name: z.string(),
        status: z.enum(['met', 'needs_improvement']),
        feedback: z.string()
      }))
    }),
    ats: z.object({
...
`;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if ((choice.finish_reason as any) === 'error') {
        const errDetail = (choice as any).error || (choice as any).message || {};
      ...
      try {
        return analysisResponseSchema.parse(jsonParsed);
      } catch (zodErr: any) {
      ...
      throw new Error(
        `OpenRouter API: ${model} request failed: ${error.message || error.toString()}`
      );
    }
  }
